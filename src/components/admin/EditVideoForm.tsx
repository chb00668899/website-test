...
export default function EditVideoForm({ video, videoId }: { video: Video; videoId: string }) {
  const router = useRouter();

  const [title, setTitle] = useState(video.title);
  const [description, setDescription] = useState(video.description || '');
  const [ossUrl, setOssUrl] = useState(video.oss_url);
  const [thumbnailUrl, setThumbnailUrl] = useState(video.thumbnail_url);
  const [duration, setDuration] = useState(video.duration ? video.duration.toString() : '');
  const [status, setStatus] = useState<'published' | 'draft'>(video.status as 'published' | 'draft');
              ...
              {ossUrl && (
                <div className="mt-4">
                  ...
                  <div className="mt-2 rounded overflow-hidden">
                    <video src={ossUrl} controls className="w-full rounded" />
                  </div>
                </div>
              )}
            </Card>
